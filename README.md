  ...
  采用PoolParty的第一种方式TpWorkerFactory进行线程池注入，用未文档化的Nt函数修改线程池的回调，具体先是拿到句柄表->枚举句柄->劫持句柄 -> 跨进程Dump到本进程 -> 查询对应的TpWorkerFactory句   柄 ->修改StartRoutine到shellcode ->VirtualAlloc&&WriteProcessMemory
  经测试，程序逻辑一切正常。<br><br>
  ...
  注意：本代码仅用于教育用途，可能会损害你的机器，请在虚拟机下运行并在24小时内删除，且运行后果自负，下载本代码视为同意该协议<br><br>
  =======================================================================================================<br><br>
